...
import { Calendar, Github, ExternalLink, Code2, Star, Clock, Users, ArrowUpRight } from "lucide-react";
...
export function ProjectContent({ project, isModal }: ProjectContentProps) {
  const content = (
    <div className={isModal ? "p-0" : "max-w-4xl mx-auto py-4"}>
      <header className="mb-10">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 border-y border-white/5 py-6">
          <div className="space-y-1">
            <div className="text-[9px] font-mono text-muted-foreground uppercase tracking-widest">STARS::COUNT</div>
          ...
          <div className="space-y-1">
            <div className="text-[9px] font-mono text-muted-foreground uppercase tracking-widest">LAST_SYNC::EPOCH</div>
            <div className="flex items-center gap-2 font-mono text-sm truncate">
                <Clock className="w-3.5 h-3.5 text-primary" />
                {project.stats?.lastCommit ? formatDistanceToNow(new Date(project.stats.lastCommit), { addSuffix: true }).toUpperCase() : "N/A"}
            </div>
          </div>

          <div className="space-y-1">
            <div className="text-[9px] font-mono text-muted-foreground uppercase tracking-widest">REPO::ACCESS</div>
            <div className="flex items-center gap-3">
                {project.links?.github && (
                <a href={project.links.github} target="_blank" className="text-xs font-mono hover:text-primary transition-colors flex items-center gap-1 uppercase">
          ...
          <div className="space-y-1">
            <div className="text-[9px] font-mono text-muted-foreground uppercase tracking-widest">SITE::LIVE</div>
            <div className="flex items-center gap-3">
                {project.links?.demo ? (
                <a href={project.links.demo} target="_blank" className="text-xs font-mono hover:text-primary transition-colors flex items-center gap-1 uppercase">
                    Demo <ArrowUpRight className="w-2.5 h-2.5" />
                </a>
                ) : <span className="text-[10px] text-muted-foreground/30 font-mono">OFFLINE</span>}
            </div>
          </div>
        </div>
      </header>
      ...
      <div className="prose prose-invert max-w-none prose-h1:text-2xl prose-h1:mt-12 prose-h2:text-xl prose-h2:mt-10 prose-p:text-muted-foreground/90 prose-p:leading-relaxed prose-p:text-lg">
        {project.content ? (
          ...
          <div className="bg-white/5 border border-white/5 p-6 rounded-sm">
            <p className="font-mono text-xs uppercase tracking-widest text-primary mb-4">[ DESCRIPTION_LOG ]</p>
      ...
      <div className="mt-16 pt-10 border-t border-white/5">
          <h4 className="text-[10px] font-mono font-bold uppercase tracking-[0.2em] text-muted-foreground/50 mb-6">DEPENDENCY_GRAPH / TECH_STACK</h4>
          <div className="flex flex-wrap gap-2">
            {project.techStack?.map((tech: string) => (
                <Badge key={tech} variant="outline" className="text-[10px] font-mono border-white/10 px-3 py-1 bg-white/[0.02] rounded-none">
                    {tech}
                </Badge>
            ))}
          </div>
  ...
  if (isModal) {
    return <ScrollArea className="max-h-[85vh] px-8 py-10">{content}</ScrollArea>;
  }
